...
const $parse_repeat25 = (input, pos, parentResults) => {
    const results = []
    let loc = pos
    let match = null
...
const action_textLine = ({ indent, text }) => ({ indent, text: [ indent, ...text.flat(), "\n" ] })
...
const action_insert = ({ index }, _, { secret }) => ({ [secret]: index })
...
const action_restAttr = ({ indent, value }) => ({ indent, pair: value })
...
const parse_attr = (input, pos) => {
    ...
    ;[loc, match] = $parse_opt22(input, loc)
    if (match === none) { location(pos); return [pos, none] }
    results.push(match)
    results.value = match
...
const action_inlineAttrs = ({ first, rest }) => [
first,
...rest.map(
item => item[3]
)
...
const parse_inlineAttrs = (input, pos) => {
    ...
    ;[loc, match] = $parse_repeat25(input, loc, results)
    if (match === none) { location(pos); return [pos, none] }
    results.push(match)
    results.rest = match
...
const parse_attrName = (input, pos) => {
    ...
    ;[loc, match] = $parse_or26(input, loc)
    if (match === none) { location(pos); return [pos, none] }
    results.push(match)
